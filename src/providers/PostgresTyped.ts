...
interface BaseOptions<T extends JSToPostgresTypeMap> {
    ...
    debug:
        | boolean
        | ((connection: number, query: string, parameters: any[]) => void);
    /** Transform hooks */
    transform: {
...
declare namespace postgres {
    ...
    interface Options<T extends JSToPostgresTypeMap>
        extends Partial<BaseOptions<T>> {
    ...
    interface ParsedOptions<T extends JSToPostgresTypeMap>
    ...
    interface ArrayParameter<
        T extends SerializableParameter[] = SerializableParameter[]
    ...
    interface ResultQueryMeta<T extends number | null, U>
        extends ResultMeta<T> {
        columns: ColumnList<U>;
    }

    type ExecutionResult<T> = [] & ResultQueryMeta<number, T>;
    type RowList<T extends readonly Row[]> = T &
        ResultQueryMeta<T["length"], keyof T[number]>;

    interface PendingQuery<TRow extends readonly Row[]>
        extends Promise<RowList<TRow>> {
        stream(
            cb: (
                row: TRow[number],
                result: ExecutionResult<TRow[number]>
            ) => void
        ): Promise<ExecutionResult<keyof TRow[number]>>;
        cursor(
            cb: (row: TRow[number]) => void
        ): Promise<ExecutionResult<keyof TRow[number]>>;
        cursor(
            size: 1,
            cb: (row: TRow[number]) => void
    ...
    interface Sql<TTypes extends JSToPostgresTypeMap> {
         ...
         */
        <
            T extends HelperSerializable,
            U extends (keyof (T extends any[] ? T[number] : T))[]
        >(
            objOrArray: T,
            ...keys: U
        ): Helper<T, U>;
        ...
        types: {
            [name in keyof TTypes]: TTypes[name] extends (...args: any) => any
                ? (
                      ...args: Parameters<TTypes[name]>
                  ) => postgres.Parameter<ReturnType<TTypes[name]>>
    ...
    interface TransactionSql<TTypes extends JSToPostgresTypeMap>
        extends Sql<TTypes> {
        savepoint<T>(
            cb: (sql: TransactionSql<TTypes>) => T | Promise<T>
        ): Promise<UnwrapPromiseArray<T>>;
